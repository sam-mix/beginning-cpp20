...
double geometric_mean(std::span<const double> data) {
  ...
  double product{1};
  for (auto value : data) product *= value;
...
double math::averages::rms(std::span<const double> data) {
  ...
  double sum_squares{};
  for (auto value : data) sum_squares += square(value);
...
double averages::median(std::span<const double> data) {
  ...
  for (auto value : data) sorted.push_back(value);
...
void quicksort(std::vector<double>& data, size_t start, size_t end) {
  ...
  if (current > start)
    quicksort(data, start, current - 1);  // Sort left subset if exists
  if (end > current + 1)
    quicksort(data, current + 1, end);  // Sort right subset if exists
}
}  // namespace