...
void show_data(const double data[], size_t count = 1,
               const std::string& title = "Data Values", size_t width = 10,
               size_t perLine = 5);
...
int main() {
  double samples[]{11.0,  23.0, 13.0, 4.0,   57.0,  36.0,
                   317.0, 88.0, 9.0,  100.0, 121.0, 12.0};
  ...
  show_data(samples, count, "Normalized Values",
            12);  // Output normalized values
}

// Outputs an array of double values
void show_data(const double data[], size_t count, const std::string& title,
...
const double* smallest(const double data[], size_t count) {
  ...
  for (size_t i{1}; i < count; ++i)
    if (data[index_min] > data[i]) index_min = i;

  return &data[index_min];
}

double* shift_range(double data[], size_t count, double delta) {
  for (size_t i{}; i < count; ++i) data[i] += delta;
  return data;
}
...
const double* largest(const double data[], size_t count) {
  ...
  size_t index_max{};
  for (size_t i{1}; i < count; ++i)
    if (data[index_max] < data[i]) index_max = i;
...
double* scale_range(double data[], size_t count, double divisor) {
  ...
  for (size_t i{}; i < count; ++i) data[i] /= divisor;
  return data;
}
