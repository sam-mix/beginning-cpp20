...
class Box {
 public:
  ...
  std::partial_ordering operator<=>(const Box& otherBox) const {
    std::cout
        << "1. std::partial_ordering operator<=>(const Box& otherBox) const "
        << std::endl;
    return volume() <=> otherBox.volume();
  }
  std::partial_ordering operator<=>(double otherVolume) const {
    std::cout
        << "2. std::partial_ordering operator<=>(double otherVolume) const "
        << std::endl;
  ...
  bool operator==(const Box& otherBox) const {
    std::cout << "3.  bool operator==(const Box& otherBox) const" << std::endl;
    return volume() == otherBox.volume();
  };
