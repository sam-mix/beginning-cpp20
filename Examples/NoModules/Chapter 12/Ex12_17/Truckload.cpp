...
// Constructor - one Box (moved to source file to gain access to definition of
// Package)
Truckload::Truckload(SharedBox box) { m_head = m_tail = new Package{box}; }
...
SharedBox Truckload::getNextBox() {
  ...
  return m_current ? m_current->getBox()
                   : nullptr;  // Return its box (or nullptr...).
}
...
bool Truckload::removeBox(SharedBox boxToRemove) {
  ...
  while (current) {
    if (current->getBox() == boxToRemove)  // We found the Box!
    {
      ...
      current->setNext(
          nullptr);    // Disconnect the current Package from the list
    ...
    current =
        current->getNext();  //  - then move current along to the next Package
  }
